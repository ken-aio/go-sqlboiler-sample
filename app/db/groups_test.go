...
func testGroupToManyGroupMembers(t *testing.T) {
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadGroupMembers(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
var (
	groupDBTypes = map[string]string{`ID`: `integer`, `Name`: `text`, `Description`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_            = bytes.MinRead
)
